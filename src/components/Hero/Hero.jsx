...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative w-full px-4 sm:px-8 lg:px-24 xl:px-40 my-12 md:my-24 lg:my-28 xl:my-36 scroll-mt-20 xl:scroll-mt-24">
      ...
      {/* Purple semi-circle */}
      <PurpleSemicircle className="absolute -z-10 top-[25%] right-2 md:right-[5%] lg:right-[8%] xl:right-[10%] w-10 h-5 md:w-24 md:h-12 lg:w-32 lg:h-16 xl:w-[155.76px] xl:h-[77.88px]" />

      {/* Centered content */}
      <div className="relative z-10 text-center">
        <h1
          ref={headingRef}
          className={`font-display font-normal text-(--text-primary) text-4xl sm:text-5xl md:text-7xl lg:text-6xl xl:text-8xl leading-snug md:leading-tight lg:leading-tight xl:leading-31.5 ${revealClass(headingVisible)}`}
        >
          <span className="sm:block">
            The{' '}
            <ZigZag lineClassName="left-[8%] w-full top-4/5">thinkers</ZigZag>
            {' '}and{' '}
          </span>
          <span className="sm:block">
            doers were{' '}
          ...
          <span className="sm:block">
            the{' '}
            <GreenPill className="relative px-5 sm:px-7 lg:px-8 xl:px-9 pb-2 sm:pb-3 lg:pb-3 xl:pb-4">status</GreenPill>
            {' '}Quo with
          </span>
        </h1>
        ...
        <p
          ref={paraRef}
          className={`mt-5 lg:mt-6 xl:mt-7 text-(--text-primary) mx-auto text-sm sm:text-base lg:text-xl xl:text-2xl max-w-sm sm:max-w-xl lg:max-w-none delay-150 ${revealClass(paraVisible)}`}
        >
